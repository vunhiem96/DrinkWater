...
class SettingFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View =
            inflater.inflate(com.vunhiem.drinkwater.R.layout.fragment_setting, container, false)
    ...
    private fun onClick() {
        ...
        ln_daily_gold.setOnClickListener {
            val mDialogView =
                LayoutInflater.from(context).inflate(R.layout.dialog_setdailygold, null)
            ...
            val mAlertDialog = mBuilder!!.show()
            val y = context?.let { it1 -> AppConfig.getGoldDrink(it1) }!!.replace(
                "[^\\d.]".toRegex(),
                ""
            ).toInt()
...
//            }
            val mDialogView =
                LayoutInflater.from(context!!).inflate(R.layout.dialog_interval_time, null)
...
//            }
            btnAccept.setOnClickListener {
                if (temp == 1) {
                    ...
                    tv_interval_setting.text = "2 hours"
                }
//                else if (temp == 5) {
...
//            .setTitle("Login Form")
        ...
        ln_weight_setting.setOnClickListener {
            ...
            val mAlertDialog = mBuilder!!.show()
            val y =
                context?.let { it1 -> AppConfig.getWeight(it1) }!!.replace("[^\\d.]".toRegex(), "")
                    .toFloat()
...
//                context?.let { it1 -> AppConfig.setWeight(tv_weight_setting.text.toString(), it1) }
        ...
        ln_wakeup_time.setOnClickListener {
            val mDialogView =
                LayoutInflater.from(context).inflate(R.layout.dialog_time_select, null)
            ...
            mAlertDialog.btn_accept_time.setOnClickListener {
                ...
                alarmMgr4 = context!!.getSystemService(Context.ALARM_SERVICE) as AlarmManager
                alarmIntent4 = Intent(context, WakeUpReceiver::class.java).let { intent ->
                ...
                val calendar4: Calendar = Calendar.getInstance().apply {
                    ...
                    set(Calendar.HOUR_OF_DAY, hhwake)
                    set(Calendar.MINUTE, mmwake)
                }
                ...
                context?.let { it1 ->
                    AppConfig.setWakeUp(
                        tv_wakeup_time_setting.text.toString(),
                        it1
                    )
                }
                mAlertDialog.dismiss()
        ...
        ln_bed_time.setOnClickListener {
            ...
            mAlertDialog.btn_accept_time.setOnClickListener {
                ...
                alarmMgr3 = context!!.getSystemService(Context.ALARM_SERVICE) as AlarmManager
                alarmIntent3 = Intent(context, BedReceiver::class.java).let { intent ->
                    PendingIntent.getBroadcast(context, 2, intent, 0)
                }
                val calendar3: Calendar = Calendar.getInstance().apply {
                    timeInMillis = System.currentTimeMillis()
                    set(Calendar.HOUR_OF_DAY, hhbed)
                    set(Calendar.MINUTE, mmbed)
                ...
                context?.let { it1 ->
                    AppConfig.setBedTime(
                        tv_bedtime_setting.text.toString(),
                        it1
                    )
                }
                mAlertDialog.dismiss()
